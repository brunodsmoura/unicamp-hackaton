				...
				<g:if test="${anuncioInstance?.contratante}">
					<li class="fieldcontain">
						<span id="contratante-label" class="property-label"><g:message code="anuncio.contratante.label" default="Contratante" /></span>
							...
							<span class="property-value" aria-labelledby="contratante-label"><g:link controller="contratante" action="show" id="${anuncioInstance?.contratante?.id}">${anuncioInstance?.contratante?.nome?.encodeAsHTML()}</g:link></span>
